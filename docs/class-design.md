...
## 10. P3-03結果のレビュー観点接続（P4-02 完了）

P4-02 の完了条件「肥大化防止、責務分離、MVP逸脱防止のレビュー観点を定義する」を満たすため、
P3-03 の方針をレビュー観点へ接続し、合否判定手順を明文化する。
...
## 11. 設計レビュー観点（P4-02 完了条件）

本章は、`肥大化防止 / 責務分離 / MVP逸脱防止` を客観判定するためのレビュー用チェックリストである。  
各項目は `Yes=合格 / No=要修正` とし、1件でも No があればレビュー不合格とする。

### 11-1. 肥大化防止（過剰設計の抑制）

| ID | 判定質問（Yes/No） | 合格基準 |
|---|---|---|
| B-01 | `SqlExplanationService` 以外に SQL句解析専用クラスを追加していないか | クラス追加が必要な理由が、現行3句（SELECT/FROM/WHERE）の範囲で説明不要である（=追加なし） |
| B-02 | DTO は `SqlExplanationResult` と `ExtractedClauses` の最小構成を維持しているか | 句ごとの細分化DTO（例: `SelectClauseInfo`）が追加されていない |
| B-03 | 公開メソッドは `Explain(string sql)` の単一入口を維持しているか | UI層から呼ぶ公開I/Fが増えていない |

### 11-2. 責務分離（UI とロジックの分離）

| ID | 判定質問（Yes/No） | 合格基準 |
|---|---|---|
| S-01 | `MainWindow` に SQL解析ロジック（句抽出・文生成）が実装されていないか | `MainWindow` は入力取得・結果表示・メッセージ更新に限定されている |
| S-02 | `SqlExplanationService` は UI依存情報（色、フォント、コントロール名）を持っていないか | 戻り値は文言/成否/警告などの論理情報のみで構成される |
| S-03 | エラー/警告の表示判断は UI層、判定材料の生成はサービス層で分離されているか | 表示可否の最終判断が `MainWindow` 側にある |

### 11-3. MVP逸脱防止（スコープ逸脱の抑制）

| ID | 判定質問（Yes/No） | 合格基準 |
|---|---|---|
| M-01 | 対応句が MVP定義（SELECT/FROM/WHERE）を超えていないか | JOIN/HAVING/ORDER BY 等は未対応として扱う |
| M-02 | 入出力I/Fが SQL説明の最小要件から逸脱していないか | DB接続、実行計画取得、外部API連携が追加されていない |
| M-03 | 画面は単画面構成を維持しているか | 追加画面や設定ダイアログが必須機能として導入されていない |

### 11-4. レビュー実施手順（運用）

1. 変更差分から `MainWindow` と `SqlExplanationService` の責務境界を確認する。  
2. 11-1〜11-3 の全質問を Yes/No で採点する。  
3. すべて Yes の場合のみ「P4-02 合格」と判定する。  
4. No がある場合は、該当IDを修正チケットへ転記して再レビューする。
